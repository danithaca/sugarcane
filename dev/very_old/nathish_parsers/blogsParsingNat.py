...
#Wordpress Needs to be worked on a lot 
def process_wordpress_post( file_name, verbose = False):
	...
	### Grab title
		resultTitle = doc.xpath("//h1[@class='post-title']")
	...
	### Grab categories 
		resultCategories = doc.xpath("//span[@class='categories']")
...
def process_typepad_post(file_name, verbose = False):
	...
	### Grab title
		resultTitle = doc.xpath("//h3[@class='entry-header']")
	...
	### Grab categories Make sure this is what we are looking for 

		resultCategories = doc.xpath("//div[@class='entry-footer']")
...
def process_newsvine_post( file_name, verbose = False):
	...
	### Grab title
		resultTitle = doc.xpath("//h1[@class='gl_headline']")
	...
	### Grab number of comments
		resultNumComments = doc.xpath("//div[@class='commentSource']")
...
def process_livejournal_post( file_name ):
	...
	try:
	# Grab main body
		result = doc.xpath("//div[@class='asset-body']")
	...
	###Get length
		...
		post_xml.set( "post_char", char)
		"""
	### Grab title: Come back to this function afterwards
		resultTitle = doc.xpath("//h1[@class='gl_headline']")
	...
	### Grab date 
		resultDate = doc.xpath("//h2[@class='asset-name page-header2']")
	...
	### Grab time (already included in date)
		resultTime = doc.xpath("//abbr[@class='datetime']")
